...
function ManageStudentProfilePage2() {
    ...
    const getStudent = async () => {
        try {
          const response = await axios.get(`http://localhost:8080/account/getByAccountid?accountid=${32}`);
    ...
    const updateAccount = async () => {
        try {
          ...
          console.log(response.data); // log the data to the console
          const student = await axios.get(`http://localhost:8080/student/getByStudentId?studentid=${33}`, {
            firstname: firstname,
            gender: gender,
            birthday: birthday,
            program: program,
